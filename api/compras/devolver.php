<?php
/**
 * POST /api/compras/devolver
 * Devolve para ajuste. Body: { "id": int, "motivo": string }
 */
...
if ($sc['status'] !== 'aguardando_aprovacao') { responderErro('Status não permite devolução.', 422); }

$pdo->prepare(
    'UPDATE solicitacoes_compra SET status = \'devolvido\', motivo_reprovacao = :motivo WHERE id = :id'
)->execute(['motivo' => trim($dados['motivo']), 'id' => $id]);

registrarHistoricoCompra($pdo, $id, $usuarioId, $payload['nome'] ?? '', 'Devolvida para ajuste', $dados['motivo']);

$pdo->prepare(
    'INSERT INTO notificacoes (usuario_id, os_id, tipo, titulo, mensagem) VALUES (:uid, NULL, :tipo, :titulo, :msg)'
)->execute([
    'uid'   => (int) $sc['solicitante_id'],
    'tipo'  => 'compra_devolvida',
    'titulo'=> 'Solicitação Devolvida para Ajuste',
    'msg'   => 'Sua solicitação ' . $sc['numero'] . ' foi devolvida para ajuste: ' . $dados['motivo'],
]);

responderSucesso(['mensagem' => 'Solicitação devolvida para ajuste.']);
