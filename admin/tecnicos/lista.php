...
$pdo = obterConexao();
$tecnicos = $pdo->query(
    "SELECT u.id, u.nome, u.email, u.telefone, u.ativo, u.foto_perfil,
     ...
     FROM usuarios u WHERE u.perfil = 'tecnico' ORDER BY u.nome"
)->fetchAll();
        ...
        <tr id="row-<?= (int) $t['id'] ?>">
          ...
          <td>
            <div class="acoes-tabela">
              <button class="btn-icone" title="Editar" onclick="abrirModalEditar(<?= htmlspecialchars(json_encode(['id'=>(int)$t['id'],'nome'=>$t['nome'],'email'=>$t['email'],'telefone'=>$t['telefone']??'','ativo'=>(bool)$t['ativo']])) ?>)"><?= ic('editar', 15) ?></button>
...
async function salvarEdicao() {
  ...
  try {
    await apiPost('/tecnicos/editar.php', {
